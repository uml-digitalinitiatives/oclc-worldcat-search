...
export class DiscoveryQuery {
    ...
    removeHoldingFilterFormat = (value: string): this => {
        this.holdingsFilterFormat = this.holdingsFilterFormat.filter((format) => format.trim().toLowerCase() !== value.trim().toLowerCase());
        return this;
    }
    ...
    removeHeldBySymbol = (value: string): this => {
        this.heldBySymbol = this.heldBySymbol.filter((symbol) => symbol.trim().toLowerCase() !== value.trim().toLowerCase());
        return this;
    }
    ...
    removeHeldByInstitutionId = (value: string): this => {
        this.heldByInstitutionId = this.heldByInstitutionId.filter((id) => id.trim().toLowerCase() !== value.trim().toLowerCase());
        return this;
    }
    ...
    removeHeldByLibraryType = (value: string): this => {
        this.heldByLibraryType = this.heldByLibraryType.filter((type) => type.trim().toLowerCase() !== value.trim().toLowerCase());
        return this;
    }
    ...
    toSearchString = (): JSON => {
      ...
      for (const key in json) {
        if (json[key] === null || json[key] === '' || (typeof json[key] === 'object' && json[key].length === 0) || json[key] === false) {
...
export const formatBriefRecordToJson = (record: BriefRecordInterface, filterLocal = false): string[][] => {
  const holdings = [];
  const baseRecord = [
    record.mmsId,
    record.oclcNumber,
    ...
    (record.mergedOclcNumbers !== undefined && record.mergedOclcNumbers.length > 0 ? record.mergedOclcNumbers.join(', ') : ""),
  ...
  if (record.institutionHolding === undefined || record.institutionHolding.briefHoldings === undefined || record.institutionHolding.briefHoldings.length === 0) {
    ...
    for (const holding of record.institutionHolding.briefHoldings) {
